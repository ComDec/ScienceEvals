...
class AlreadySaidThat(SolverEval):
    ...
    def eval_sample(self, solver: Solver, sample: dict, rng: random.Random) -> None:
        ...
        distractor_data = deque(
            rng.sample(self.distractor_data, k=self.num_distractors)
        )

        conversation_metrics = self._conversation_loop(
            solver, words, distractor_data, rng
        )
    ...
    def _conversation_loop(
        ...
        while convo_metrics["num_turns"] < self.max_turns:
            # conversation
            distracting_words = (
                words_from_solver.union(words_from_distractors)
                if self.adversarial
                else set()
            )
            ...
            elif convo_metrics["violation_occurred"] or (
                mistake_made and distractor_added is None
            ):
    ...
    def _compute_agg_metrics(self, logged_metrics: list[dict]) -> dict:
        ...
        agg_metrics = {
            ...
            "false_positive_rate": np.mean(
                [x["was_false_pos"] for x in logged_metrics]
            ),
            "false_negative_rate": np.mean(
                [x["was_false_neg"] for x in logged_metrics]
            ),
            # distractor stats
            ...
            "violation_rate": np.mean(
                [x["violation_occurred"] for x in logged_metrics]
            ),
        }
