...
from evals.elsuite.ballots.prompts import (first_message_influencer,
                                           make_decision_prompt)
from evals.elsuite.ballots.utils import (ALL_DOUBLE_NEWLINE_TOKS,
                                         BASE_REMOVED_TOKS, CONTROL_DIRECTION,
                                         DOUBLE_NEWLINE_TOKS, LOGIT_BIAS_MAX,
                                         LOGIT_BIAS_MIN, NO_DIRECTION,
                                         VOTER_DECISION_TOKS, YES_DIRECTION,
                                         chat_to_text, format_prompt,
                                         get_influencer_prompt,
                                         get_voter_prompt, reverse_roles,
                                         toks_to_id)
...
class BallotsEval(Eval):
    ...
    def eval_sample(self, sample: Any, *_):
            ...
            if is_chat_model(fn.model):
                ...
                if "logit_bias" not in kwargs:
                    if fn.model in {"gpt-4-base"}:
                        ...
                        kwargs["logit_bias"] = {
                            id: LOGIT_BIAS_MIN
                            for id in toks_to_id(fn.model, DOUBLE_NEWLINE_TOKS)
                        }
        ...
        response = query(
            ...
            logit_bias={
                id: LOGIT_BIAS_MAX
                for id in toks_to_id(self.voter_fn.model, VOTER_DECISION_TOKS)
            },
            fn=self.voter_fn,
    ...
    def run(self, recorder):
        ...
        return {
            ...
            "vote_yes_rate__target_control": compute_mean(
                "is_vote_yes__target_control"
            ),
            "se__vote_yes_rate__target_control": compute_stderr(
                "is_vote_yes__target_control"
            ),
