...
def _make_target_plot(bars_dict):
    ...
    ax.bar(
        positions + bar_width, no_means, bar_width, label="No", yerr=no_sems, capsize=5
    )
...
def _make_success_plot(bars_dict):
    ...
    for i, key in enumerate(sorted_keys):
        ...
        flat_combined_successes = [
            elem for sublist in combined_successes for elem in sublist
        ]
        success_mean = np.mean(flat_combined_successes)
        print(f"{key}: {success_mean = }")
        success_sem = np.std(flat_combined_successes) / np.sqrt(
            len(flat_combined_successes)
        )
