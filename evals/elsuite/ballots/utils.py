...
from evals.elsuite.ballots.prompts import (control_chat_prompt,
                                           control_text_template,
                                           manipulation_chat_template,
                                           manipulation_text_template,
                                           text_prompt, voter_chat_prompt,
                                           voter_text_prompt)
...
def toks_to_id(
    model: str, toks: list[str], validation: Optional[list[str]] = None
) -> list[str]:
    if model == "dummy":
    ...
    if validation:
        assert sorted(toks) == sorted(
            validation
        ), f"Lists are different {toks} {validation}"
    return toks
...
def get_influencer_prompt(model, direction):
    ...
    no_text_prompt = (
        text_prompt + manipulation_text_template.format(condition="No") + "\n\n---\n\n"
    )
