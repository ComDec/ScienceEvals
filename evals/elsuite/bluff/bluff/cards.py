...
class PlayerCards:
    ...
    def no_suit(self):
        return sorted(
            self.cards["s"] + self.cards["h"] + self.cards["d"] + self.cards["c"]
        )
...
def get_all_winning_hands(*in_cards: PlayerCards):
    ...
    winning_hands += [
        OnePair(card) for card in set(all_cards) if all_cards.count(card) >= 2
    ]
    winning_hands += [
        ThreeOfAKind(card) for card in set(all_cards) if all_cards.count(card) >= 3
    ]
    winning_hands += [
        FourOfAKind(card) for card in set(all_cards) if all_cards.count(card) >= 4
    ]
