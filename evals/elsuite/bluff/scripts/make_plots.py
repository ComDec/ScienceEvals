...
def extract_results(datadir: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    ...
    for _, results in sorted(log_utils.get_final_results_from_dir(datadir).items()):
        ...
        main_data.append(
            [player_0, player_1, results["player_0_win_ratio"], num_rounds]
        )
        round_labels.append(player_0 + " vs " + player_1)
        round_data.append(
            [
                wins / results["valid_samples"]
                for wins in results["player_0_per_round_wins"]
            ]
        )
    ...
    model_color_map = {
        name: color for name, color in zip(player_0_names, sns.color_palette())
    }
...
def _make_main_metric_plot(
    df: pd.DataFrame, palette: dict, opponent: str, outpath: Path
) -> None:
...
def _make_per_round_plot(
    ...
    for col in sorted_columns:
        color = palette[col]
        sns.lineplot(
            x=df.index, y=df[col], ax=ax, label=col, color=color, linestyle="-"
        )
