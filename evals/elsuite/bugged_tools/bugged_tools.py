...
def explicit_error(
    tool_task_state: ToolTaskState, non_bugged_func: Callable
) -> ToolResult:
...
def offset_numeric(
    correct_output: Union[int, float], offset: Union[int, float]
) -> ToolResult:
    """
...
def large_offset(
    ...
    def _large_offset_string(correct_output: str) -> ToolResult:
        ...
        pos = np.random.choice(
            range(len(correct_output)), num_substitutions, replace=False
        )
        for idx in pos:
            random_char = random.choice(
                string.ascii_letters + string.digits + string.punctuation
            ...
            correct_output = (
                correct_output[:idx] + random_char + correct_output[idx + 1 :]
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Small offset for {Double} on input {x}: {small_offset(example_task_state, Double())}"
    )
    print(
        f"Large offset for {Double} on input {x}: {large_offset(example_task_state, Double())}"
    )
    print(
        f"Random output for {Double} on input {x}: {random_output(example_task_state, Double())}"
    )
    print(
