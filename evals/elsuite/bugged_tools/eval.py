...
from evals.elsuite.bugged_tools.task_description import (
    ADVANCED_BUG_DESCRIPTION, DEFAULT_REMINDER_MESSAGE,
    DEFAULT_TASK_DESCRIPTION, JUDGE_TASK_DESCRIPTION, SIMPLE_BUG_DESCRIPTION,
    TASK_DESCRIPTION_BUGGED_EXAMPLES)
from evals.elsuite.bugged_tools.tools import ALL_TOOLS, BuggedTool
from evals.elsuite.bugged_tools.utils import (precision_recall_fscore,
                                              try_cast_from_str)
...
class BuggedTools(SolverEval):
    ...
    def eval_sample(self, solver: Solver, sample: Any, rng: random.Random):
        ...
        if not isinstance(solver, DummySolver):
            metrics["solver_predicted_bug"] = self._solver_predicted_bug(
                sample, all_messages
            )
        else:
    ...
    def _judge_find_bugs(self, all_messages: Sequence[Message]):
        judge = registry.make_completion_fn(
            "gpt-4-0613", extra_options={"temperature": 0.0}
        )
        ...
        if "[YES]" in response:
            ...
            logging.warn(
                f"Judge didn't output correct flag but instead generated:\n{response}"
            )
            return False

    def _solver_predicted_bug(self, sample: Any, all_messages: Sequence[Message]):
        bugged_pattern = r"\(@Bugged: (.+?)\)"
        predicted_bugged_tools = self._find_flag_from_assistant(
            all_messages, bugged_pattern
        )
    ...
    def _solver_used_bugged_input(self, sample: Any, all_messages: Sequence[Message]):
        ...
        tool_inputs_used = [
            strip_and_cast(i, type(bugged_input)) for i in tool_inputs_used
        ]
