...
from evals.elsuite.cant_do_that_anymore.chess.utils import (
    Move, get_other_player_id, get_path_between_coords, parse_piece)
...
class Board:
    ...
    def _update_board(self, move: Move):
        ...
        piece_color, piece_id = parse_piece(
            self.board_state, start_coord[0], start_coord[1]
        )
        ...
        if piece_id == 0 and target_piece_color == "E":
            ...
            self.board_state[target_en_passant_piece[0]][
                target_en_passant_piece[1]
            ] = "E"
        ...
        if move.promotion is not None:
            self.board_state[target_coord[0]][
                target_coord[1]
            ] = f"{piece_color}{move.promotion}"

    def _get_player_moves(
        self, player_id: str, previous_moves: Sequence[Move]
    ) -> Sequence[Move]:
        """
    ...
    def _is_king_in_check(self, player_id: str) -> bool:
        ...
        king_capturing_moves = self._filter_for_king_capturing_moves(
            other_player_moves, player_id
        )
...
class BoardController:
    ...
    def __init__(
        ...
        self.board = Board(
            board_init(), piece_id_to_instance, piece_str_to_id, piece_id_to_str
        )
    ...
    def get_player_legal_moves(self, player_id: str) -> Sequence[str]:
        ...
        legal_moves = [
            self.notation_parser._move_to_str(i, self.board.board_state)
            for i in legal_moves
        ]
        return legal_moves

    def _filter_to_prevent_pinning(
        self, moves: Sequence[Move], player_id: str
    ) -> Sequence[Move]:
        """
