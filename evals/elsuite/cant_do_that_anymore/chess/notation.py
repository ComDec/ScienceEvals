...
class NotationParser:
    ...
    @abstractmethod
    def _str_to_move(
        self, s: str, board_state: Sequence[Sequence[int]], player_id: str
    ) -> Move:
        raise NotImplementedError()

    @abstractmethod
    def _move_to_str(
        self, move: Move, board_state: Sequence[Sequence[int]], player_id: str
    ) -> str:
        raise NotImplementedError()
...
class AlgebraicNotationParser(NotationParser):
    ...
    def _str_to_move(self, s: str, board_state: Sequence[Sequence[int]]) -> Move:
        ...
        # Parse start coord
        start_row = (
            row_idx_swap(int(match.group(2))) if match.group(2) is not None else None
        )
        start_col = (
            letter_to_num[match.group(1)] if match.group(1) is not None else None
        )
    ...
    def _move_to_str(self, move: Move, board_state: Sequence[Sequence[int]]) -> str:
        ...
        target = (
            f"{num_to_letter[target_coord[1]]}{row_idx_swap(target_coord[0])}".lower()
        )
        out_str += target
