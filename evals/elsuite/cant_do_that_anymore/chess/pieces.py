...
from evals.elsuite.cant_do_that_anymore.chess.utils import (
    Move, coord_within_board, get_other_player_id, get_path_between_coords,
    has_piece_been_moved, move_crosses_pieces, parse_piece)
...
class Piece:
    ...
    def get_piece_moves(
        ...
        if self.captures_like_pawn:
            possible_transformations = (
                self._remove_illegal_pawn_capture_transformations(
                    board_state,
                    player_id,
                    start_coord,
                    possible_transformations,
        ...
        if self.can_castle:
            piece_moves += self._get_castling_possible_moves(
                board_state, player_id, previous_moves
            )
    ...
    def _remove_illegal_pawn_capture_transformations(
        ...
        capturing_moves = [
            [move[0] * forward_direction, move[1]] for move in capturing_moves
        ]
        for move in possible_transformations + capturing_moves:
            ...
            target_piece_color, target_piece_id = parse_piece(
                board_state, new_row_idx, new_col_idx
            )

            if target_piece_color == "E" and move not in capturing_moves:
                new_possible_transformations.append(move)
            elif (
                target_piece_color == get_other_player_id(player_id)
                and move in capturing_moves
            ):
                new_possible_transformations.append(move)
    ...
    def _get_castling_possible_moves(
        ...
        def _can_pieces_castle(
            king_init_coord: Sequence[int],
            rook_init_coord: Sequence[int],
            init_rook_id: int,
        ) -> Sequence[Move]:
            if init_rook_id != 3:
                return []

            if has_piece_been_moved(
                king_init_coord, previous_moves
            ) or has_piece_been_moved(rook_init_coord, previous_moves):
                return []
            ...
            king_to_rook_path = get_path_between_coords(
                king_init_coord, rook_init_coord
            )
        ...
        _, init_king_id = parse_piece(
            board_state, king_init_coord[0], king_init_coord[1]
        )
        if init_king_id != 5:
            return castling_moves
