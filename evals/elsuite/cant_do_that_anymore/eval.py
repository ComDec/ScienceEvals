...
from evals.elsuite.cant_do_that_anymore.chess.utils import (
    capturing_same_color, move_within_board, same_color_piece_at_move_start)
from evals.elsuite.cant_do_that_anymore.defaults import (
    TASK_DESCRIPTION, TASK_DESCRIPTION_VARIANT)
from evals.elsuite.cant_do_that_anymore.utils import (
    construct_messages, get_binary_avg, get_dataset_path,
...
class CantDoThatAnymore(SolverEval):
    ...
    def eval_sample(self, solver: Solver, sample: Any, rng: random.Random):
        ...
        solver_result_variant = get_solver_pred(
            TASK_DESCRIPTION_VARIANT, variant_controller
        )
    ...
    def run(self, recorder: RecorderBase) -> dict[str, Union[float, int]]:
        ...
        predicted_move_in_variant_proportion = get_binary_avg(
            metrics, "predicted_move_in_variant"
        )

        avg_num_previous_moves = sum([i["num_previous_moves"] for i in metrics]) / len(
            metrics
        )
    ...
    def get_violations(
        ...
        try:
            ...
            piece_capturing_same_color = capturing_same_color(
                controller.board.board_state, move
            )
