...
from evals.elsuite.cant_do_that_anymore.chess.utils import (
    Move, coord_within_board, move_crosses_pieces, parse_piece)
from evals.elsuite.cant_do_that_anymore.utils import (dump_sequence_to_jsonl,
                                                      initialise_boards)
...
def get_bishop_diagonal_moves(
    controller: BoardController, player_id: str
) -> Sequence[str]:
    """
    ...
    for row_idx, col_idx in bishop_coords:
        for transformation in DIAGONAL_MOVES:
            ...
            piece_color, _ = parse_piece(
                controller.board.board_state, new_coord[0], new_coord[1]
            )
            ...
            move = controller.notation_parser._move_to_str(
...
def find_specific_moves_in_game(
    ...
    stockfish.set_fen_position(
        "rnnqknnr/pppppppp/8/8/8/8/PPPPPPPP/RNNQKNNR w KQkq - 0 1"
    )
    previous_moves = []
    player_id = "W"
