...
from evals.utils.log_utils import (extract_individual_results, extract_spec,
...
def compute_num_previous_bishop_moves(previous_moves: Sequence[str]) -> int:
    ...
    for move in previous_moves:
        ...
        _, piece_id = parse_piece(
            controller.board.board_state, start_coord[0], start_coord[1]
        )
        if piece_id == 2:
...
def plot_diagonal_bishop_results(df: pd.DataFrame, out_dir: Path):
    ...
    df = (
        df.groupby(["solver_path", "num_previous_bishop_moves"])
        .agg(agg_operations)
        .reset_index()
    )
