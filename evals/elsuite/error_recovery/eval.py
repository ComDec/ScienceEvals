...
from evals.elsuite.error_recovery.defaults import (
    DEFAULT_FINAL_ANSWER_MESSAGE, DEFAULT_MISTAKE_MESSAGE,
    DEFAULT_TASK_DESCRIPTION, TASK_SPECIFIC_EXTRACTION_INFO)
...
class ErrorRecovery(SolverEval):
    def __init__(
        ...
        **kwargs,
        ...
        self.mistake_notif_pos: Optional[MistakeNotifPos] = (
    ...
    def eval_sample(
        self, solver: Solver, sample: Sample, rng: random.Random, extra_logging=None
    ):
        task = sample.task
    ...
    def _get_answer(
        ...
        answer = self._extract_final_answer(
            solver=solver, task_state=task_state, sample=sample
        )
        return answer
    ...
    def run(self, recorder: evals.record.Recorder):
        ...
        for task in all_tasks:
            ...
            NR_correct_rate = len(
                [i for i in filtered_metrics if i["NR_correct"]]
            ) / len(filtered_metrics)
            CR_correct_rate = len(
                [i for i in filtered_metrics if i["CR_correct"]]
            ) / len(filtered_metrics)
            IR_correct_rate = len(
                [i for i in filtered_metrics if i["IR_correct"]]
            ) / len(filtered_metrics)
    ...
    def _extract_final_answer(
        self, solver: Solver, task_state: TaskState, sample: Sample
    ):
