...
class AverageBaseline(Solver):
    ...
    * Make three guesses: round/floor/ceil of average values for neighboring numbers
    ...
    def _get_guess(
        self, test_inputs, known_values: dict[int, int], guess_round_ix
    ) -> str:
        known_values = {
            x: y
            for x, y in known_values.items()
            if x - 1 in test_inputs or x + 1 in test_inputs
        }
        ...
        for i, test_input in enumerate(test_inputs):
            try:
                ...
                return (
                    "Unfortunately I don't have enough data to make a guess, will pass."
                )
...
class FullKnowledge(Solver):
    ...
    def __init__(self, mode: str, samples_jsonl: str, registry: Registry):
        ...
        self.mode = mode
        self._all_samples = self._get_samples(
            samples_jsonl, registry._registry_paths[0]
        )
