...
class CurrentState:
    ...
    def guess_update(
        ...
        if self.mode == "easy":
            for test, guess, correct in zip(
                self.test_inputs, guessed_ints, expected_ints
            ):
                if guess == correct:
...
class FunctionDeductionEval(SolverEval):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            completion_fns, seed=seed, samples_jsonl=samples_jsonl, *args, **kwargs
        )
        ...
        for round_ix in range(self.n_rounds):
            ...
            try:
                ...
                if len(ints) == 1:
                    ...
                    if result is None:
                        answer = prompts.test_input_not_allowed.format(
                            inputs=test_inputs
                        )
                    else:
    ...
    def run(self, recorder: evals.record.Recorder):
        ...
        adjusted_rounds = [
            x["num_rounds"] or self.failed_sample_rounds for x in metrics
        ]
    ...
    def _get_success_metrics(self, metrics):
        ...
        return {
            ...
            "samples": len(metrics),
            "avg_success_rounds": (
                round(sum(x["num_rounds"] for x in success) / len(success), 2)
                if success
                else None
            ),
            "sem_avg_success_rounds": (
                self._calculate_sem([x["num_rounds"] for x in success])
                if success
                else None
            ),
        }
    ...
    def _get_sample_std(self, metrics):
        ...
        for sample_ix in sample_ixs:
            sample_metrics = [
                metric for metric in metrics if metric["sample_ix"] == sample_ix
            ]
            sample_adjusted = [
                metric["num_rounds"] or self.failed_sample_rounds
            ...
            solved_ratio = sum(
                1 for metric in sample_metrics if metric["success"]
            ) / len(sample_metrics)
        ...
        return {
            "avg_sample_rounds_std_adjusted": (
                sum(adjusted) / len(adjusted) if adjusted else None
            ),
            "avg_sample_rounds_std_no_failed": (
                sum(no_failed) / len(no_failed) if no_failed else None
            ),
            ...
            "solved_ratio_if_any_solved": (
                sum(solved_ratio_if_any_solved) / len(solved_ratio_if_any_solved)
    ...
    def _get_complexity_tests(self, metrics):
        ...
        result = {
            ...
            "not_solved_avg_complexity": (
                sum(not_solved) / len(not_solved) if not_solved else None
            ),
        }
    ...
    def _bad_guess_answer(self, test_inputs, guessed, expected) -> str:
        ...
        if self.mode == "hard":
            return (
                "This is not the correct answer. At least one of the values is wrong."
            )
        elif self.mode == "easy":
            if len(correct) == 0:
