...
from evals.elsuite.hr_ml_agent_bench.benchmarks.parkinsons_disease.env.train import \
    smapep1
...
def get_score(submission_dir: Path) -> float:
    ...
    assert (
        answers_fname.exists()
    ), f"Expected {answers_fname.as_posixs()} to exist, but it doesn't."
