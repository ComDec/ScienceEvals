...
def add_padding(
    X: np.ndarray, pad_size: Union[int, list, tuple], pad_val: int = 0
) -> np.ndarray:
    """
    ...
    assert (
        len(X.shape) == 4
    ), "Input image should be form of [Batch, Width, Height, Channels]"
    ...
    return np.pad(
        X, pad_width=pad_width, mode="constant", constant_values=(pad_val, pad_val)
    )
...
class Conv2DLayer:
    ...
    def __init__(
        ...
        self.biases = np.random.randn(
            1, 1, 1, num_filters
        )  # 1 Bias per Channel/feature/filter
        self.stride = stride
        ...
        for h in range(h_new):
            for w in range(w_new):
                ...
                for filter_index in range(num_of_filters_new):
                    ...
                    conv_result = (
                        np.sum(image_slice * kernel_matrix, axis=(1, 2, 3))
                        + bias.flatten()
                    )
