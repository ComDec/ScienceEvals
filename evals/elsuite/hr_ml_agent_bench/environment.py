...
from evals.elsuite.hr_ml_agent_bench.high_level_actions import \
    HIGH_LEVEL_ACTIONS
from evals.elsuite.hr_ml_agent_bench.low_level_actions import LOW_LEVEL_ACTIONS
from evals.elsuite.hr_ml_agent_bench.prepare_task import (get_research_problem,
                                                          prepare_task)
from evals.elsuite.hr_ml_agent_bench.schema import (Action,
                                                    EnhancedJSONEncoder,
                                                    EnvException, LLMError,
                                                    Step, TooLongPromptError,
                                                    Trace)
...
class Environment:
    ...
    def _setup_log_dir(self):
        ...
        if os.path.exists(os.path.join(self.log_dir, "tool_logs")):
            logger.info(
                f"tools_log_dir {os.path.join(self.log_dir, 'tool_logs')} already exists"
            )
        else:
        ...
        if os.path.exists(os.path.join(self.log_dir, "traces")):
            logger.info(
                f"tools_log_dir {os.path.join(self.log_dir, 'traces')} already exists"
            )
        else:
    ...
    def _initialize_trace(self):
        if self.resume:
            ...
            prev_trace = from_dict(
                data_class=Trace,
                data=json.load(
                    open(os.path.join(self.resume, "env_log", "trace.json"), "r")
                ),
            )
    ...
    def execute(
        self, action: Action, max_seconds_per_step: Optional[int] = None
    ) -> str:
