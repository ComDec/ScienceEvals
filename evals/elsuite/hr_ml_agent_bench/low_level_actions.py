...
from evals.elsuite.hr_ml_agent_bench.schema import (Action, ActionInfo,
                                                    EnvException, Step)
from evals.elsuite.hr_ml_agent_bench.utils import \
    get_gpu_with_most_available_memory as get_device
...
def record_low_level_step(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if "trace" not in new_kwargs["kwargs"]:
            logger.info(
                "Warning: trace not found in kwargs; not recording low level step."
            )
            logger.info(func)
