...
for commit in commits_to_include:
    log_dir = (
        get_root_dir() / "elsuite" / "hr_ml_agent_bench" / "scripts" / "logs" / commit
    )
...
task_to_category = {
    task: category for category, tasks in task_categories.items() for task in tasks
}
...
df_only_direct = df_non_raw[
    df_non_raw["solver_id"].str.contains("direct|human|naive", regex=True)
]
df_only_direct
...
rl_report_summary_table["category"] = rl_report_summary_table["task"].map(
    task_to_category
)
...
rl_report_summary_table = rl_report_summary_table.sort_values(
    by=["category", "task", "solver_id"]
)
