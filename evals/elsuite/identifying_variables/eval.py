...
from evals.elsuite.identifying_variables.metrics import (compute_fallout,
                                                         compute_nDCG,
                                                         compute_recall)
...
from evals.elsuite.identifying_variables.utils import (json_to_sample,
                                                       parse_solver_preds)
...
class IdentifyingVariables(SolverEval):
    ...
    def _compute_grouped_metrics(self, metrics: List[Dict]) -> Dict[str, float]:
        ...
        for log_entry in metrics:
            causal_graph = nx.from_dict_of_lists(
                log_entry["causal_graph"], create_using=nx.DiGraph
            )
            ...
            group_to_bin = {
                ...
                "max_corr_depth": (
                    graph_utils.find_farthest_node(causal_graph, dep_var)[1]
            ...
            for group, g_bin in group_to_bin.items():
                if g_bin is not None:
                    for metric, raw_metric in zip(
                        metric_to_agg_func.keys(), raw_metric_names
                    ):
                        grouped_metrics[f"{metric}-{group}-{g_bin}"].append(
                            log_entry[raw_metric]
                        )

        # aggregate
        grouped_metrics = {
            k: (
                metric_to_agg_func[k.split("-")[0]](v)
    ...
    def _evaluate_sample(
        self, preds: Optional[Answer], gold: Answer, num_not_ctrl: int
    ) -> Dict:
        """
        ...
        """
        hyp_valid_correct = (
            preds.valid_hypothesis == gold.valid_hypothesis if preds else False
        )
    ...
    def _ctrl_vars_fallout(
        self, preds: List[str], gold: List[str], num_not_ctrl: int
    ) -> float:
    ...
    def _get_samples(self) -> List[Sample]:
        if self.debug:
            return gen_samples(
                n_samples=1000, signal_noise_ratio=None, np_rng=self.np_rng
            )
