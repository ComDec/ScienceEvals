...
def val_and_count_roots(
    ...
    if min_roots is not None:
        assert (
            min_roots <= n_nodes
        ), "Total roots must be less than or equal to the number of nodes"
...
def gen_random_forest_tree_size(
    ...
    assert (
        tree_size <= num_nodes
    ), "Tree size must be less than or equal to the number of nodes"
    ...
    for node in shuffled_nodes:
        ...
        if total_edges is not None:
            ...
            if remaining_edges <= 0:
                break
            num_edges = np_rng.integers(
                0, min(remaining_edges, len(possible_children)) + 1
            )
        else:
            num_edges = np_rng.integers(0, len(possible_children) + 1)
