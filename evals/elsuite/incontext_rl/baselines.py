...
class QlearningSolver(Solver):
    ...
    def reduce_epsilon(self, episode_number):
        ...
        ) * np.exp(-self.decay_rate * episode_number)
    ...
    def _solve(self, task_state: TaskState, **kwargs) -> SolverResult:
        ...
        assert (
            cs.action_space_n is not None
        ), "Environment must have discrete action space"

        if self.q_table is None:
            print("Initializing Q-table")
            self.initialize_q_table(
                observation_space_size=cs.observation_space_n,
                action_space_size=cs.action_space_n,
            )
        ...
        action = self.select_action(
            state=cs.observations[-1], action_space=cs.action_space
        )
        response = f"[SELECT: {action}]"
