...
from evals.elsuite.incontext_rl.defaults import (reset_msg, reward_counter,
                                                 step_counter, step_result,
                                                 step_result_reset,
...
class InContextRl(SolverEval):
    ...
    def eval_sample(self, solver: Solver, sample: Any, rng: random.Random):
        ...
        for _ in range(self.max_steps):
            ...
            if terminated or truncated:
                ...
                content = self._format_step_message(
                    action, next_observation, reward, sample
                )
                ts.messages += [Message(role="user", content=content)]
        ...
        evals.record.record_metrics(
            ...
            invalid_response_rate=(
                ts.current_state.invalid_responses / ts.current_state.total_responses
        ...
        for metric in metrics:
            env_result = {
                ...
                "metrics": {
                    ...
                    "average_reward_last_5_episodes": metric[
                        "average_reward_last_5_episodes"
                    ],
                    "average_reward_last_10_episodes": metric[
                        "average_reward_last_10_episodes"
                    ],
                    "average_reward_last_20_episodes": metric[
                        "average_reward_last_20_episodes"
                    ],
                    "average_reward_last_50_episodes": metric[
                        "average_reward_last_50_episodes"
                    ],
    ...
    def _try_get_valid_action(
        ...
        logger.info(
            f"Exceeded maximum invalid action attempts ({self.max_invalid_responses})."
        )
        return None
    ...
    def _add_message_to_task_state(
        self, task_state: TaskState, role: str, content: str
    ) -> None:
        """
    ...
    def _format_step_message(
        ...
        return template.substitute(
            action=action, next_observation=observation_desc, reward=reward
        )
