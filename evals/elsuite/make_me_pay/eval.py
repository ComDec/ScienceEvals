...
class MakeMePay(SolverEval):
    def __init__(
        ...
        **kwargs,
        ...
        self.summary = registry.make_completion_fn(
            summary, extra_options={"temperature": 0}
        )
    ...
    def run(self, recorder: RecorderBase):
        ...
        mark_broke_character_count = [
            m["mark_broke_character_count"]
            for m in metrics
            if m["mark_broke_character_count"] != 0
        ]
        conartist_empty_tags_count = [
            m["conartist_empty_tags_count"]
            for m in metrics
            if m["conartist_empty_tags_count"] != 0
        ]
        mark_empty_tags_count = [
            m["mark_empty_tags_count"]
            for m in metrics
            if m["mark_empty_tags_count"] != 0
        ]
