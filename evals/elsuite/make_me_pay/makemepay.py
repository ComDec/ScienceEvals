...
from evals.elsuite.make_me_pay.task_description import (WITHDRAW_PROMPT,
                                                        task_description_dict)
from evals.elsuite.make_me_pay.utils import (format_msgs, is_system_msg,
...
def generate_reply_for(
    ...
    if isinstance(max_turns_or_time, int):
        task_description = task_description.format(
            duration=f"{max_turns_or_time} minutes"
        )
    elif isinstance(max_turns_or_time, float):
        task_description = task_description.format(
            duration=f"{max_turns_or_time} turns"
        )
    else:
        assert (
            False
...
def run_eval(
    ...
    **kwargs,
    ...
    while cap_not_reached():
        ...
        conversation, conartist_broke_character, conartist_output_empty_tags = (
            generate_reply_for(
        ...
        (
            conversation,
            delta_mark_broke_character,
            delta_mark_empty_tags,
        ) = generate_reply_for(
            conversation=conversation,
            role="mark",
            solver=mark_solver,
        ...
        mark_empty_tags_count = (
            delta_mark_empty_tags + 1
            if delta_mark_empty_tags
            else mark_empty_tags_count
        )
    ...
    ), f"Got {donation_amount} instead of float."
    assert isinstance(num_replies, int)
    assert isinstance(elapsed, float)
    assert (
        isinstance(conartist_broke_character_count, int)
        and conartist_broke_character_count >= 0
    )
    assert (
        isinstance(conartist_empty_tags_count, int) and conartist_empty_tags_count >= 0
    )
    assert (
        isinstance(mark_broke_character_count, int) and mark_broke_character_count >= 0
    )
