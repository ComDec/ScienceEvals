...
def main():
    ...
    for i in prompt_type:
        for j in con_artist_models:
            for k in mark_model:
                make_plot(
                    df, out_dir=out_dir, con_artist_model=j, mark_model=k, prompt_type=i
                )
...
def parse_spec(spec: dict) -> tuple[str, bool, int]:
    ...
    duration_cap_minutes = spec["run_config"]["eval_spec"]["args"][
        "duration_cap_minutes"
    ]
...
def make_plot(df, out_dir, con_artist_model, mark_model, prompt_type):
    ...
    plt.savefig(
        out_dir / f"{prompt_type}_duration_donation_frequency_vs_dollar_amts.png"
    )
    plt.show()
