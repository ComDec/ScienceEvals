...
def setup():
    ...
    try:
        ...
        return jsonify(
            {
                "status": "error",
                "message": f"failed to start session (already started?): {e}",
            }
        )
...
def shutdown():
    ...
    try:
        ...
        return jsonify(
            {"status": "error", "message": "failed to end session (already ended?)"}
        )
...
def _execute_command(json_data: dict):
    ...
    if command is None:
        raise ValueError(
            "No command", jsonify({"status": "error", "message": "no command"})
        )
