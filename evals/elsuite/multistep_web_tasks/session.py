...
from evals.elsuite.multistep_web_tasks.utils import \
    load_experiment_config_from_dict
from evals.elsuite.multistep_web_tasks.webarena.core.utils import (
    BashBrowserExperimentConfig, BrowserExperimentConfig)
...
class Session:
    ...
    def __enter__(self):
        ...
        try:
            ...
            logger.info(
                "Waiting for containers to start up, could take several minutes..."
            )
    ...
    def register_container(self, container_name: ServiceIdentifier, container: docker.models.containers.Container) -> None:  # type: ignore
        assert (
            self.containers is not None
        ), "Session must be entered before registering containers"
    ...
    def _setup_gitlab_environment(self) -> docker.models.containers.Container:  # type: ignore
        entrypoint_file = str(
            (Path(__file__).parent / "docker/gitlab/entrypoint.sh").resolve()
        )
    ...
    def _setup_wikipedia_environment(self) -> docker.models.containers.Container:  # type: ignore
        ...
        if not wikipedia_path.is_file():
            logger.warning(
                f"wikipedia zim not found at {wikipedia_path}, downloading..."
            )
            try:
                download_to_file(
    ...
    def _get_image(
        ...
        if url is not None and cache_file is not None:
            ...
            try:
                ...
                logger.warning(
                    f"Error loading from downloaded {image_name} tar from {url}: {e}"
                )
    ...
    def teardown_docker_environments(self) -> None:
        ...
        for container_name in list(self.containers.keys()):
            logger.info(
                f"Removing container {container_name}: {self.containers[container_name]}"
            )
    ...
    def _is_container_ready(
        ...
        assert (
            self.containers is not None
        ), "Session must be entered before checking containers"
        ...
        while time.time() < end_time:
            try:
                ...
                logger.debug(
                    f"While checking {url} for {container_name}, got exception: {e}"
                )
