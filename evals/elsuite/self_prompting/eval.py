...
from evals.elsuite.self_prompting.task_description import (
    sample_in_token, task_description_template)
...
class SelfPrompting(SolverEval):
    def __init__(
        ...
        **kwargs,
        ...
        for tasker_model in self.tasker_models:
            self.tasker_completion_fns[tasker_model] = registry.make_completion_fn(
                tasker_model
            )
    ...
    def _run_tasking(self, sample: Any, *_):
        ...
        if sample_in_token in sample["model_instruction"]:
            ...
            full_prompt = sample["model_instruction"].replace(
                sample_in_token, sample["input"]
            )
        else:
        ...
        fuzzy = (
            1
            if tasker_output in sample["output"] or sample["output"] in tasker_output
            else 0
        )
    ...
    def _calculate_improvement_wrt_baseline(
        ...
        try:
            ...
            logger.warn(
                "SKIPPING IMPROVEMENT METRICS. (Failed to validate baseline spec.)"
            )
            return {}
        ...
        def compute_mean_tasker(key, tasker_model):
            return np.mean(
                [
                    ...
                    if metric["tasker_model"] == tasker_model
                ]
            )
