...
from evals.elsuite.skill_acquisition.utils import (
    PROMPTS, answer_detected, get_accuracy, get_average_bleu_score,
    get_average_invalid_retrieval_calls, get_average_retrieval_calls,
    get_average_retrieval_precision, get_bleu_score,
    get_bootstrap_accuracy_std, get_question_type, get_std_of_difference,
    process_answer, process_view_instruction, render_intermediate_prompt,
...
class SkillAcquisition(SolverEval):
    def __init__(
        ...
        **kwargs,
        ...
        self.knowledge_base_directory = self._prefix_registry_path(
    ...
    def eval_sample(
        self, solver: Solver, sample: Dict, rng: random.Random
    ) -> Dict[str, Any]:
    ...
    def _eval_non_retrieval_sample(
        self, solver: Solver, sample: Dict, *_
    ) -> Dict[str, Any]:
    ...
    def _eval_retrieval_sample(
        ...
        if answer_detected(output):
            ...
            answer = "NO ANSWER DETECTED"
            logging.debug(
                f"Model timed out after {metrics['current_replies']} replies."
            )
    ...
    def run(self, recorder: evals.record.Recorder) -> dict[str, Union[float, int]]:
        ...
        num_translation_samples = len(
            [
                ...
                if result["question_type"] == "translation"
            ]
        )
        num_non_translation_samples = len(
            [
                result
                for result in retrieval_results
                if result["question_type"] == "non-translation"
            ]
        )
        ...
        result = {
            ...
            "delta_std": delta_std,
            "average_retrieval_precision": get_average_retrieval_precision(
                retrieval_results
            ),
            "average_non_retrieval_bleu_score": get_average_bleu_score(
    ...
    def _view_content(
        ...
        return (
            file_content_by_section[section_title],
            sections_visible_to_model,
            sections_viewed,
        )
    ...
    def _conversation_loop(
        ...
        while (
            not answer_detected(output)
            and metrics["current_replies"] < self.max_replies
        ):
            if metrics["current_replies"] == 0:
                ...
                task_state.task_description = (
                    task_state.task_description
                    + "\n\n"
                    + PROMPTS["retrieval_instructions"].format(
                        list_of_files=self.files_available
                    )
                )
            if len(sections_viewed.items()) > 0:
                intermediate_prompt = render_intermediate_prompt(sections_viewed)
                task_state.messages += [
                    Message(role="system", content=intermediate_prompt)
                ]
            ...
            if view_instruction_detected(output):
                ...
                if (
                    section is not None
                    and section not in sections_visible_to_model[file]
                ):
                    task_state.messages += [
                        Message(
                            role="system",
                ...
                content, sections_visible_to_model, sections_viewed = (
                    self._view_content(
                ...
                task_state.messages += [
                    Message(
                        ...
                        content=PROMPTS["present_content"].format(
                            file=file,
                            section=(
                                section if section is not None else "Table of Contents"
                            ),
                            content=content,
                        ),
                    ),
                ...
                if section is None:
                    logger.debug(
                        f"Model viewed table of contents for file {file}: {content}"
                    )
                else:
