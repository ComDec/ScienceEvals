...
def process_miskito_page():
    ...
    for idx in range(1, 11):
        ...
        for i, div in enumerate(divs_with_specific_style):
            if (
                i == 0 and idx == 1
            ):  # First section of first lesson is not in the same format.
                ...
                json.dumps(
                    {
                        "input": [
                            {
                                "role": "user",
                                "content": instructions + qa_dict["question"],
                            }
                        ],
...
for article in as_text:
    for heading in article:
        ...
        heading_name = (
            md(str(hsoup.find("span", class_="mw-headline").contents[0]))
            .replace("*", "")
            .strip()
        )
...
with open(file, "w") as f:
    for heading, content in sections_by_heading.items():
        f.write(
            json.dumps({"title": heading, "content": content}, ensure_ascii=False)
            + "\n"
        )
# %%
