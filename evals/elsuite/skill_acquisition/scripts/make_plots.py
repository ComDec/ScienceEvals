...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        ...
        df.rename(
            columns={"average_retrieval_precision": "average_correct_calls"},
            inplace=True,
        )
        df["correct_call_rate"] = (
            df["average_correct_calls"] / df["average_retrieval_calls"]
        )
        ...
        for metric in ["overall_accuracy", "bleu_score"]:
            make_side_bar_plot(
                df, out_dir / f"{dataset}_{metric}.png", metric, dataset=dataset
            )
        ...
        for type, closedbook, openbook in [
            ...
            for idx, row in grouped.iterrows():
                ...
                rel_improvement_score = (openbook_score - closedbook_score) / (
                    1 - closedbook_score
                )
