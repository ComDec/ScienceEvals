...
class Runner:
    ...
    def run(self) -> RunnerResult:
        ...
        while turn < self.max_turns:
            ...
            if parsed_solver_result.tool_calls == [] and final_answer is None:
                ...
                task_state = self._add_eval_message(
                    task_state, solver_result, content=content
                )
                turn += 1
                continue
            ...
            tool_outputs = [
                self._run_tool_call(i) for i in parsed_solver_result.tool_calls
            ]
    ...
    def _parse_tool_calls(self, output: str) -> Optional[list[ToolCall]]:
        ...
        for tool_name, tool_message in tool_message_matches:
            ...
            try:
                ...
                logger.warn(
                    f"Solver tried to call '{tool_name}' tool which doesn't exist!"
                )
                continue
    ...
    def _run_tool_call(self, tool_call: ToolCall) -> ToolCall:
        ...
        task_description = f"Your name is {tool_name}. A description of your purpose is shown below:\n{tool_desc}"
