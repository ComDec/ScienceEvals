            ...
            return [
                {
                    ...
                    "brevity_score": complexity.calculate_brevity_score(
                        self.dataset[i]
                    ),
                }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if cache_path.exists() and use_cache:
            ...
            print(
                f"{cache_path} not found. Loading from HF {hf_path}/{dataset_name}/{split}"
            )
...
class RandomCharAndNumberDataset(BaseTextDataset):
    def __init__(self, n_samples, seed=0, lengths=[5, 10, 20, 50, 100]):
        ...
        number_dataset = RandomNumberDataset(
            n_samples - (n_samples // 2), seed, lengths
        )
...
class RandomWordsDataset(BaseTextDataset):
    def __init__(self, n_samples, seed=0, lengths=[5, 10, 20, 50, 100]):
        ...
        for i in range(n_samples):
            length = self.rng.choice(lengths)
            random_words_string = " ".join(
                self.rng.choice(word_list) for _ in range(length)
            )
    ...
    def __getitem__(self, idx):
        if isinstance(idx, slice):
            ...
            return [
                {
                    ...
                    "type": self.type,
                    "length": len(self.dataset[i]["prompt"])
                    + len(self.dataset[i]["output"]),
                }
                for i in range(start, stop, step)
            ]
        else:
            ...
            return {
                ...
                "type": self.type,
                "length": len(self.dataset[idx]["prompt"])
                + len(self.dataset[idx]["output"]),
            }
