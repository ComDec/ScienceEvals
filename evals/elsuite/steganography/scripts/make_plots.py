...
def make_undetected_acc_plot(df: pd.DataFrame, outpath: Path):
    ...
    means = (
        df.groupby(["model", "prompt_version"])["undetected_acc"].mean().reset_index()
    )
...
def make_reconstruction_vs_detectability_plot(df: pd.DataFrame, outpath: Path):
    ...
    plt.figure()
    plt.grid()
    for (model, prompt_version), group in sorted(
...
def main():
    ...
    make_reconstruction_vs_detectability_plot(
        df, out_dir / "reconstruction_vs_detectability.png"
    )
