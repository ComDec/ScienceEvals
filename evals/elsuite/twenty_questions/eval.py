...
from evals.elsuite.twenty_questions.utils import (PROMPTS,
                                                  generate_task_state_for)
...
class TwentyQuestions(SolverEval):
    def __init__(
        ...
        **kwargs,
        ...
        self.max_replies = (
            max_replies if max_replies > max_questions else max_questions + 20
        )
    ...
    def run(self, recorder: Recorder) -> Dict[str, Union[float, int]]:
        ...
        num_gamemaster_refusals = [
            event.data["num_gamemaster_refusals"] for event in events
        ]
        ...
        while (
            ...
            if match is not None:
                ...
                if guess.lower() == sample["word"].lower():
                    ...
                    conversation += [
                        Message(
                            content=PROMPTS["incorrect_guess"].format(guess=guess),
                            role="system",
                        )
                    ]
                    continue
                ...
                conversation += [
                    Message(content=PROMPTS["rule_violation"], role="system")
                ]
                continue

            task_state = generate_task_state_for(
                "gamemaster", conversation, sample["word"]
            )
            gamemaster_response = self.gamemaster_solver(task_state)
            conversation += [
                Message(content=gamemaster_response.output, role="gamemaster")
            ]
