...
class OpenAIAssistantsSolver(Solver):
    ...
    def _solve(
        ...
        **kwargs,
        ...
        last_assistant_msg_idx = self._get_last_assistant_message_idx(
            task_state.messages
        )
        new_msgs_start_idx = (
            last_assistant_msg_idx + 1 if last_assistant_msg_idx is not None else 0
        )

        # Add new messages to Thread
        last_msg_sent = None
        for idx, message in enumerate(task_state.messages[new_msgs_start_idx:]):
            user_message = self._convert_to_user_message(
                message
            )  # API only allows "user" messages
        ...
        for message in messages:
            for content in message.content:
                if content.type == "text":
                    ...
                    logging.warning(
                        "Assistant sent an image, but this is not yet supported."
                    )
                else:
                    raise NotImplementedError(
                        f"Content type {content.type} not supported."
                    )
        output_text = "\n".join(contents)
        ...
        record_sampling(
            prompt=[Message("system", task_state.task_description)]
            + task_state.messages,
            sampled=[output_text],
            model=self.model,
            tools=self.tools,
        ...
        with FILE_CACHE_LOCK:
            ...
            try:
                ...
                if "Invalid file format." in e.message:
                    logging.warning(
                        f"{file_path} rejected due to invalid file format, skipping."
                    )
                    return None
                else:
                    raise e
