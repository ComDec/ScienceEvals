...
from evals.solvers.providers.openai.openai_assistants_solver import (
    FILE_CACHE, OpenAIAssistantsSolver)
    ...
    solver_result = solver(
        TaskState(
            ...
            messages=[
                Message(
                    role="user",
                    content="Please calculate the sqrt of 145145 to 3 decimal places.",
                ),
            ],
        ),
    )
...
def test_code_interpreter_file(
    dummy_recorder, dummy_data_file, code_interpreter_solver
):
...
def test_file_cache(dummy_recorder, dummy_data_file, retrieval_solver):
    ...
    assert (
        tmpfile_path in FILE_CACHE
    ), f"File should be cached. Cached files: {FILE_CACHE}"
