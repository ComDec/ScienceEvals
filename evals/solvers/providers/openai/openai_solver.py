...
class OpenAISolver(Solver):
    ...
    def _solve(self, task_state: TaskState, **kwargs) -> SolverResult:
        ...
        try:
            if self._is_chat_model(self.model):
                ...
                completion_result = self.completion_fn(
                    prompt=prompt, stop=stop_sequences, **kwargs
                )
    ...
    def _render_completion_prompt(self, msgs: list[dict[str, str]]) -> str:
        ...
        if (
            self.continue_last_assistant_msg
            and len(msgs) > 0
    ...
    def _preprocess_completion_fn_options(self) -> dict:
        ...
        if self.valid_answers is not None and len(self.valid_answers) > 0:
            self.completion_fn_options["extra_options"]["logit_bias"] = (
                self._make_logit_bias(self.valid_answers, model)
            )

    def _make_logit_bias(
        self, valid_answers: list[str], model: str
    ) -> dict[int, float]:
