...
class NestedSolver(Solver):
    ...
    def __init__(
        self, *, postprocessors: list[str] = [], registry=None, **solver_specs
    ):
        ...
        for name, value in solver_specs.items():
            try:
                ...
                raise ValueError(
                    f"Expected a sub-solver spec at '{name}', got '{value}'"
                )
    ...
    def copy(self: SolverType) -> SolverType:
        ...
        solver_copy = deepcopy(
            self
        )  # TODO: We should deepcopy without copying the cache
